...
- "Works offline" means the **installed** app, served over http/https and backed by the service worker. Opening `index.html` straight from disk (a `file://` URL) does render the map — MapLibre GL JS 5 loads as a plain classic script and map tiles still fetch — but it is **not a supported way to run the app**: browsers refuse to register a service worker on a `file://` origin (it is not a secure context), so there is no offline caching and no install prompt, and every `/api/*` call resolves against the filesystem instead of a backend, which disables GPX upload/sharing, POIs and snap-to-route. To run from a local checkout, serve the folder over http — the `uvicorn` command under [Optional Backend](#optional-backend-gpx-upload-and-sharing), or any static server (`python -m http.server 8000`, `npx serve`).
...
- **v2.22.0:** Reverted the vendored map engine from **MapLibre GL JS 6.3.0 back to 5.24.0**, undoing the v2.17.0 ESM migration and returning to the simpler loading path. v5 ships a **UMD bundle**, so `vendor/` holds a single `maplibre-gl.js` again instead of the three-file ESM set (`maplibre-gl.mjs` + `maplibre-gl-shared.mjs` + `maplibre-gl-worker.mjs`), and the `maplibre-boot.mjs` module shim is gone — `index.html` loads the library as a plain `defer` classic script, which sets the `maplibregl` global itself. The service worker precaches one vendor file instead of three, and `maplibre-boot.mjs` drops out of the `?v=`-stamped set (back to four stamped assets). On the backend, the `.mjs` accommodations are removed (`mimetypes.add_type`, the `STATIC_ASSET_SUFFIXES` entry and the `PUBLIC_ROOT_FILES` allowlist entry), and the Report-Only CSP's `worker-src` gains **`blob:`** — v5 builds its worker with `URL.createObjectURL(new Blob([...]))` rather than fetching a worker file, so `'self'` alone would report a violation once that policy is promoted to enforced. No app code changed: the whole MapLibre surface goes through the Leaflet-compatibility adapter in `script.js`, and every constructor, `MapOptions` key, camera/style method, event name and layer spec it uses exists unchanged in 5.24.0. The print map's `canvasContextAttributes: { preserveDrawingBuffer: true }` is deliberately **kept** — it was mislabelled as a v6 migration but is a genuine fix that applies from 5.0.0 onward, and reverting it would silently re-break the PDF export. **Note:** WebGL 1 support returns, so the map no longer requires a WebGL2-capable browser. Two upstream 6.3.0 fixes are given up in exchange: the `ImageSource` GPU-texture leak that affects the slope overlay's opacity slider, and terrain gestures anchoring to the elevation under the pointer rather than the map centre.
